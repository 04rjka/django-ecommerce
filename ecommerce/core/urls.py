from django.urls import path
from . import views

urlpatterns = [
    path("",views.home,name="home"),
    path("search/",views.search,name="search"),
    path("staff-search/",views.staff_search,name="staff_search"),
    path("signup/",views.customer_signup,name="customer_signup"),
    path("login/",views.customer_login,name="customer_login"),
    path("logout/",views.customer_logout,name="customer_logout"),
    path("staff-login/",views.staff_login,name="staff_login"),
    path("staff-logout/",views.customer_logout,name="staff_logout"),
    path("add/",views.add_product,name="add_product"),
    path("add/<int:pk>/images/",views.add_product_images,name="add_images"),
    path("edit-product/<int:pk>/",views.edit_product,name="edit_product"),
    path("edit-product/<int:pk>/images",views.edit_product_images,name="edit_product_images"),
    path("product/<int:pk>/",views.product_page,name="product_page"),
    path("profile/",views.profile,name="profile"),
    path("addtocart/<int:pk>/",views.add_to_cart,name="add_to_cart"),
    path("cart/",views.cart,name="cart"),
    path("checkout/",views.checkout,name="checkout"),
    path("remove/<int:pk>",views.remove_cart_item,name="remove"),
    path("staffhome/",views.staff_home,name="staff_home"),
    path("staffproduct/<int:pk>/",views.staff_product_page,name="staff_product_page"),
    path("inc/<int:pk>",views.increment_cart_item,name="increment"),
    path("dec/<int:pk>",views.decrement_cart_item,name="decrement"),
    path("address/",views.add_address,name="add_address"),
    path("addresses/",views.view_address,name="view_address"),
    path("delete-address/<int:pk>/",views.delete_address,name="delete_address"),
    path("order-success/<int:pk>/",views.order_success,name="order_success"),
    path("orders/",views.orders,name="orders"),
    path("process-orders/",views.process_orders,name="process_orders"),
    path("order-details/<int:pk>/",views.order_details,name="order_details"),
    path("set-order-shipped/<int:pk>/",views.set_order_shipped,name="set_order_shipped"),
    path("staff-order-details/<int:pk>/",views.staff_order_details,name="staff_order_details"),
    path("edit-address/<int:pk>/",views.edit_address,name="edit_address"),
    path("payment/<int:pk>/",views.initiate_payment,name="initiate_payment"),
    path("payment/success/",views.payment_success,name="payment_success"),
    path("cancel-order/<int:pk>/",views.cancel_order,name="cancel_order"),
    path("refund-order/<int:pk>/",views.refund_order,name="refund_order"),
]